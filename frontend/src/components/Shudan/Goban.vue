...
export default {
    ...
    props: {
        ...
        coordY: {
            ...
            default() {
                const length = 26;
                return [...Array(length)].map((_, i) => length - (length - i) + 1);
            }
        },
    ...
    computed: {
        ...
        shiftMap() {
            const { boardSize, boardWidth, boardHeight } = this;
            const shiftMap = [...Array(boardWidth * boardHeight)].map(() => helper.random(8));
        ...
        randomMap() {
            ...
            return [...Array(boardWidth * boardHeight)].map(() => helper.random(4));
        },

        boardWidth() {
            const { signMap } = this;
            return signMap.length > 0 ? signMap[0].length : 0;
        ...
        xs() {
            this.updateElementSize();
            const { boardWidth, rangeX } = this;
            return helper.range(boardWidth).slice(rangeX[0], rangeX[1] + 1);
        },

        ys() {
            this.updateElementSize();
            const { boardHeight, rangeY } = this;
            return helper.range(boardHeight).slice(rangeY[0], rangeY[1] + 1);
        },

        hoshis() {
            const { boardWidth, boardHeight} = this;
            return helper.getHoshis(boardWidth, boardHeight);
        },

        vertices() {
            const { xs, ys, fuzzyStonePlacement, shiftMap, randomMap,
                    signMap, heatMap, paintMap, markerMap, ghostStoneMap,
                    dimmedMap, selectedMap, animatedVertices} = this;
            const result = [];
            ys.forEach(y => { xs.forEach(x => {
                ...
                result.push({
                    offsetY,
                    offsetX,
                    shift: fuzzyStonePlacement ? shiftMap && shiftMap[offsetY][offsetX] : 0,
                    random: randomMap && randomMap[offsetY][offsetX],
                    sign: signMap && signMap[offsetY][offsetX],
                    heat: heatMap && heatMap[offsetY][offsetX],
                    paint: paintMap && paintMap[offsetY][offsetX],
                    marker: markerMap && markerMap[offsetY][offsetX],
                    ghostStone: ghostStoneMap && ghostStoneMap[offsetY][offsetX],
                    dimmed: dimmedMap && dimmedMap[offsetY][offsetX],
                    selected: selectedMap && selectedMap[offsetY][offsetX],
                    animate: animatedVertices &&  false// animatedVertices.some(v => v === offsetY) // TODO
                });
            });});
            return result;
        }
        ...
        <div
            ...
            <div
                ...
                <Vertex
                    ...
                    :selected="v.selected"
                    :animate="v.animate"
                    @click="$emit('click', v.offsetY, v.offsetX)"
                    @rightClick="$emit('rightClick', v.offsetY, v.offsetX)"
                    @mousedown="$emit('mousedown', v.offsetY, v.offsetX)"
                    @mouseup="$emit('mouseup', v.offsetY, v.offsetX)"
                    @mousemove="$emit('mousemove', v.offsetY, v.offsetX)"
                    @mouseenter="$emit('mouseenter', v.offsetY, v.offsetX)"
                    @mouseleave="$emit('mouseleave', v.offsetY, v.offsetX)"
                    />
            </div>
